...
class EC2:
    ...
    @staticmethod
    def instance_profile_valid(
        instance_profile_name: str = "", asg_name: str = "", iam_role_name: str = ""
    ) -> bool:
        """
    ...
    @staticmethod
    def launch_config_valid(
        launch_config_name: str = "",
        asg_name: str = "",
        expected_instance_type: str = "",
        expected_key_name: str = "",
        expected_sg_count: int = 0,
        expected_instance_profile: str = "",
    ) -> bool:
        """
        ...
        lcs = autoscaling.describe_launch_configurations(
            LaunchConfigurationNames=[launch_config_name], MaxRecords=1
        )
    ...
    @staticmethod
    def autoscaling_schedule_valid(
        asg_name: str = "",
        schedule_name: str = "",
        recurrence: str = "",
        max_size: int = 0,
    ...
    @staticmethod
    def load_balancer_target_groups_valid(
        asg_name: str = "", load_balancer_target_group_names: list = None
    ) -> bool:
        """
        ...
        if (
            load_balancer_target_group_names is None
            or len(load_balancer_target_group_names) == 0
        ):
            return len(load_balancers) == 0
        else:
            for index, target_group in enumerate(load_balancer_target_group_names):
                if not load_balancers[index].get(
                    "State"
                ) == "InService" or f"targetgroup/{target_group}" not in load_balancers[
                    index
                ].get(
